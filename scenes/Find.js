...
let createNewLobby = (user, pProps) => {
    ...
    pProps.actions.changeLobbyName(lobbyName)
    console.error(pProps)
}
            ...
            if (lobies == null) {
                ...
                allLobbies.forEach(key => {
                  ...
                  if (noOfPlayers <= max_lobby_players) {
                    ...
                    paramProps.actions.changeLobbyName(key)
                    console.error(paramProps)
                    lobbyFound = true;
                  }
                }); //allLobies.loop
              } //else
...
//export default function Game(props) {
    ...
    constructor(props){
      ...
      /*this.generateBack()
      let username = this.paramProps.first
        let userId = username + Date.now();
        let user = {
            username,
            userId: userId
        };
        findGameLobby(user, this.paramProps).then(val => {
